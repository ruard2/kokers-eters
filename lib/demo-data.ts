...
function participant(input: Record<string, unknown>) {
  return {
    ...
    isGuest: Boolean(input.isGuest || input.communityScope === "GUESTS_AND_NEWCOMERS"),
    ...
    cannotEatDays: input.cannotEatDays ? String(input.cannotEatDays) : null,
    cannotHostDays: input.cannotHostDays ? String(input.cannotHostDays) : null,
    adminNoMatch: input.adminNoMatch ? String(input.adminNoMatch) : null,
